...
export const getAdminBooks = async () => {
  try {
    ...
    if (res.success && res.results) {
      ...
      // Cache về local phòng khi offline
    ...
    console.error('getAdminBooks API failed, falling back to local bookRepository:', err);
  }
  // Fallback offline: đọc từ cache đã lưu trước đó
...
export const getBooks = async () => {
  try {
    ...
    console.error('getBooks API failed, falling back to local bookRepository:', err);
  }
  // Fallback offline: đọc từ cache đã lưu trước đó
  const cached = await bookRepository.list();
  return cached.filter((book) => book.status !== 'Bản nháp' && book.status !== 'draft');
};
  ...
  try {
    if (id && (!isNaN(id) || /^\d+$/.test(id))) {
      ...
      if (res.success && res.story) {
        const book = mapBackendBookToFrontend(res.story);
        // Cache về local phòng khi offline
        await bookRepository.save(book);
        return book;
      }
    ...
    console.error('getBookById API failed, falling back to local bookRepository:', err);
  }
  // Fallback offline: tìm trong cache đã lưu trước đó
  return bookRepository.findById(String(id));
};
...
export const completeCoinTopup = async ({
  ...
  try {
    ...
    // Hiện thông báo nạp xu thành công
    if (!result.alreadyProcessed) {
      const totalCoin = Math.max(0, Number(coin) || 0) + Math.max(0, Number(bonus) || 0);
      try {
        const { showImmediateNotification, addNotificationToStore } = await import('./notificationService');
        await showImmediateNotification(
          '💰 Nạp xu thành công!',
          `Tài khoản đã được cộng ${Number(totalCoin).toLocaleString('vi-VN')} xu.`,
          { route: '/lich-su-giao-dich', type: 'transaction' },
        );
        await addNotificationToStore(userId, {
          type: 'transaction',
          title: 'Nạp xu thành công',
          message: `Tài khoản đã được cộng ${Number(totalCoin).toLocaleString('vi-VN')} xu. Số dư: ${Number(result.balance).toLocaleString('vi-VN')} xu.`,
          route: '/lich-su-giao-dich',
        });
      } catch (notifErr) {
        console.warn('Không gửi được thông báo nạp xu:', notifErr);
      }
    }
    return result;
  } finally {
...
/**
 * Thêm 1 thông báo vào AsyncStorage (wrapper để dùng ở nhiều nơi trong service này).
 */
export const addNotification = async (userId, notifData) => {
  const { addNotificationToStore } = await import('./notificationService');
  return addNotificationToStore(userId, notifData);
};
...
export const getNotifications = async (userId) => {
  ...
  // Lọc bỏ thông báo fake cũ từ năm 2025 dựa vào prefix ID
  const isFakeId = (id) => typeof id === 'string' && 
    (id.startsWith('notification_chapter_') || 
     id.startsWith('notification_transaction_') || 
     id.startsWith('notification_system_'));

  const userItems = all.filter((item) => String(item.userId) === String(userId) && !isFakeId(item.id));
