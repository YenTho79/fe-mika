...
const getDevServerIp = () => {
  ...
  return '10.230.244.55';
};

const DEV_IP = getDevServerIp();

// Cấu hình URL ngrok (khi dùng ngrok, đặt URL tại đây; để trống nếu muốn quay về IP nội bộ)
export const NGROK_URL = 'https://keep-extending-flakily.ngrok-free.dev';

// API Base URL Configuration
export const API_BASE_URL = NGROK_URL
  ? `${NGROK_URL}/api`
  : Platform.select({
    web: 'http://10.230.244.55:8000/api',
    default: `http://${DEV_IP}:8000/api`,
  });

// Helper fetch với Timeout 20 giây (phù hợp với mạng di động và tunnel ngrok)
...
/**
 * Đăng ký Expo Push Token lên backend: POST /api/user/push-token/
 * @param {string} pushToken - ExponentPushToken[xxx]
 * @param {string} authToken - api_token của user
 * @returns {Promise<{success: boolean, message?: string}>}
 */
export async function registerPushToken(pushToken, authToken) {
  try {
    const response = await fetchWithTimeout(`${API_BASE_URL}/user/push-token/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    console.error('API registerPushToken error:', error);
    return { success: false, message: error.message };
  }
}

/**
 * Admin gửi thông báo đẩy hàng loạt: POST /api/admin/broadcast-notification/
 * @param {{ title: string, body: string, data?: object }} payload
 * @param {string} authToken - api_token của admin
 * @returns {Promise<{success: boolean, sent?: number, message?: string}>}
 */
export async function sendAdminBroadcastNotification(payload, authToken) {
  try {
    const response = await fetchWithTimeout(`${API_BASE_URL}/admin/broadcast-notification/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Token ${authToken}`,
    ...
    console.error('API sendAdminBroadcastNotification error:', error);
    return { success: false, message: error.message };
  }
}
