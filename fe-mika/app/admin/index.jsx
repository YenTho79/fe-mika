...
import { sendAdminBroadcastNotification } from '../../constants/api';
...
export default function AdminDashboard() {
  ...
  const [broadcast, setBroadcast] = useState({ title: '', body: '', sending: false, result: '' });
  ...
  return (
    <Screen padded={false} safeAreaTop={false}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <SectionHeader title="Gửi thông báo đẩy" subtitle="Broadcast đến tất cả thiết bị đã đăng ký" />
        <Card style={styles.broadcastCard}>
          <View style={styles.broadcastIconRow}>
            <View style={styles.broadcastIcon}><Ionicons name="megaphone-outline" size={22} color={colors.primary} /></View>
            <Text style={styles.broadcastHint}>Thông báo sẽ gửi đến toàn bộ user có Expo Push Token trong database.</Text>
          </View>
          <TextInput
            style={[styles.broadcastInput, { color: colors.text, borderColor: colors.outline + '40' }]}
            placeholder="Tiêu đề thông báo..."
            placeholderTextColor={colors.muted}
            value={broadcast.title}
            onChangeText={(v) => setBroadcast((p) => ({ ...p, title: v, result: '' }))}
            maxLength={80}
          />
          <TextInput
            style={[styles.broadcastInput, styles.broadcastBody, { color: colors.text, borderColor: colors.outline + '40' }]}
            placeholder="Nội dung thông báo..."
            placeholderTextColor={colors.muted}
            value={broadcast.body}
            onChangeText={(v) => setBroadcast((p) => ({ ...p, body: v, result: '' }))}
            multiline
            numberOfLines={3}
            maxLength={200}
          />
          {broadcast.result ? <Text style={[styles.broadcastResult, { color: broadcast.result.startsWith('✅') ? colors.tertiary : colors.danger }]}>{broadcast.result}</Text> : null}
          <Pressable
            style={[styles.broadcastBtn, (!broadcast.title.trim() || !broadcast.body.trim() || broadcast.sending) && styles.broadcastBtnDisabled]}
            disabled={!broadcast.title.trim() || !broadcast.body.trim() || broadcast.sending}
            onPress={async () => {
              setBroadcast((p) => ({ ...p, sending: true, result: '' }));
              try {
                const res = await sendAdminBroadcastNotification(
                  { title: broadcast.title.trim(), body: broadcast.body.trim(), data: { route: '/thong-bao', type: 'system' } },
                  data.user?.token,
                );
                setBroadcast((p) => ({
                  ...p,
                  sending: false,
                  title: res.success ? '' : p.title,
                  body: res.success ? '' : p.body,
                  result: res.success ? `✅ Đã gửi đến ${res.sent ?? 0} thiết bị!` : `❌ ${res.message || 'Gửi thất bại.'}`,
                }));
              } catch (err) {
                setBroadcast((p) => ({ ...p, sending: false, result: `❌ ${err.message}` }));
              }
            }}
          >
            <Ionicons name="send-outline" size={18} color="#fff" style={{ marginRight: spacing.sm }} />
            <Text style={styles.broadcastBtnText}>{broadcast.sending ? 'Đang gửi...' : 'Gửi thông báo'}</Text>
          </Pressable>
        </Card>
      </ScrollView>
    </Screen>
...
const getStyles = (colors) => StyleSheet.create({
  ...
  broadcastCard: { gap: spacing.md, marginBottom: spacing.xl },
  broadcastIconRow: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.md },
  broadcastIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: 'rgba(124,58,237,0.14)', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  broadcastHint: { ...typography.caption, color: colors.muted, flex: 1, lineHeight: 18 },
  broadcastInput: { borderWidth: 1, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, ...typography.body, backgroundColor: colors.surface3 },
  broadcastBody: { minHeight: 72, textAlignVertical: 'top' },
  broadcastResult: { ...typography.caption, fontWeight: '700', textAlign: 'center' },
  broadcastBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primary, borderRadius: radius.md, paddingVertical: spacing.md, paddingHorizontal: spacing.lg },
  broadcastBtnDisabled: { opacity: 0.45 },
  broadcastBtnText: { ...typography.body, color: '#fff', fontWeight: '800' },
});
